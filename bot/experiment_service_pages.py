"""Standalone experiment: process service page screenshots using the same
page-by-page Claude API approach as new_method.py.

Hardcoded to the F40PH-3C MBTA train_02 pages for now.

Usage:
    python bot/experiment_service_pages.py
"""
...
OUTPUT_CSV = os.path.join(PAGES_DIR, "experiment_pages.csv")
OUTPUT_JSON = os.path.join(PAGES_DIR, "experiment_pages.json")
...
# ---------------------------------------------------------------------------
# Prompt (identical to new_method.py _build_service_list_prompt)
# ---------------------------------------------------------------------------
def build_prompt():
    anchor_hint = ""
    ...
    return (
        ...
        "Use timestamps to determine whether a box is a new/different service.\n"
...
# ---------------------------------------------------------------------------
# Helpers (copied from new_method.py)
# ---------------------------------------------------------------------------
...
def main():
    ...
    # Phase 2: Dedup (day-wrap aware)
    # Services are chronological and can wrap past midnight.
    # Once we see a time go backwards by >12 hours, we're in a new day —
    # from that point on, matching times are NOT duplicates.
    unique_services = []
    exact_keys = set()       # (name, start_time, duration) before day wrap
    dupes_removed = 0
    ...
    for r in all_results:
        for svc in r["services"]:
            ...
            if not day_wrapped:
                ...
                # (page overlap can still cause dupes within the wrap zone)
    ...
    print(f"  Expected:              59 (manual count)")
    print(f"{'='*60}")
