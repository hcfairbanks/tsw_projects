"""Standalone experiment B: same as experiment_service_pages.py but with
the ORIGINAL prompt (no day-wrap hint) and one change:
  "Use timestamps to determine..." -> "Use the unique combination of service name, start time, and duration to determine..."

Usage:
    python bot/experiment_service_pages_b.py
"""
...
OUTPUT_CSV = os.path.join(PAGES_DIR, "experiment_pages_b.csv")
OUTPUT_JSON = os.path.join(PAGES_DIR, "experiment_pages_b.json")
...
# ---------------------------------------------------------------------------
# Prompt (original from new_method.py, with one line changed)
# ---------------------------------------------------------------------------
def build_prompt():
    anchor_hint = ""
...
def main():
    ...
    # Phase 2: Dedup (original new_method.py logic)
    unique_services = []
    exact_keys = set()
    dupes_removed = 0
