...
func WriteRouteDataJSON(w io.Writer, tts []*uasset.Timetable, opts RailsGeoJSONOptions) (int, error) {
	...
	// Prefer DisplayName + Country from the route's RouteDefinition asset
	// (stamped onto every Timetable by the extractor). Fall back to the
	// legacy CamelCase split when the RouteDefinition scan failed.
	displayName := geomTT.RouteDisplayName
	if displayName == "" {
		displayName = RouteDisplayName(geomTT.Route)
	}
	doc := map[string]any{
		"type":                     "FeatureCollection",
		"name":                     displayName,
		"route":                    geomTT.Route,
		"cross_pak_reference_name": geomTT.CrossPakReferenceName,
		"country":                  CountryNameFromCode(geomTT.CountryCode),
		// country_code is the ISO 3166-1 alpha-2 code (TSW's "UK" is
		// normalised to "GB"). The importer prefers this for matching
		// existing country rows, since the flag CSS keys off `code`
		// and bypassing the name lookup avoids stale duplicate rows.
		"country_code": CountryISOFromCode(geomTT.CountryCode),
