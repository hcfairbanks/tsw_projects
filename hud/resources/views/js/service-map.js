...
(function () {
    ...
    async function attach(map, opts) {
        ...
        // Fetch features. Two paths, with the timetable path falling back to
        // the route path:
        //
        ...
        //
        //   If that returns 404 (the blob hasn't been built yet — only a
        //   tiny fraction of timetables have it) AND we know the route id,
        //   fall through to the route's full GeoJSON. Without this fallback
        //   every timetable without a pre-built blob renders an empty layer
        //   bar — which is exactly what was happening on /map.
        //
        //   timetableId absent (route-only mode) → /api/routes/{id}/map-data
        //   directly. The filter checks below stay live but with no
        ...
        if (opts.timetableId) {
            try {
                ...
                if (rtt.ok) {
                    ...
                    preFiltered = true;
                    fetchedTtFeatures = true;
                } else if (rtt.status !== 404) {
                    console.warn('ServiceMap.attach: timetable map-features HTTP ' + rtt.status);
                }
            } catch (e) {
                console.warn('ServiceMap.attach: timetable map-features fetch failed', e);
            }
        }
        if (!fetchedTtFeatures && opts.routeId) {
            try {
